...
def main():
    ...
    while running:
        ...
        if keys[pygame.K_w] or keys[pygame.K_UP]:
            update_vector_cluster_position_up(ball_cluster, 300, dt)
        if keys[pygame.K_s] or keys[pygame.K_DOWN]:
            update_vector_cluster_position_down(ball_cluster, 300, dt)
        if keys[pygame.K_a] or keys[pygame.K_LEFT]:
            update_vector_cluster_position_left(ball_cluster, 300, dt)
        if keys[pygame.K_d] or keys[pygame.K_RIGHT]:
            update_vector_cluster_position_right(ball_cluster, 300, dt)
